...
# 使い方（具体例）
# 1) デフォルトの入出力を使う
#    python3 scripts/generate_tsukanshi_sheet.py
#
# 2) 明示的に入力JSONと出力PDFを指定する（正規スキーマJSON）
#    python3 scripts/generate_tsukanshi_sheet.py --input qa/宅建業法_暗記シート.json --output pdf/宅建業法_暗記シート.pdf
#
# 3) ユーザー作成の簡易JSON（question/answer形式）を指定する
#    python3 scripts/generate_tsukanshi_sheet.py --input qa/通関業法01.json --output pdf/通関業法01.pdf